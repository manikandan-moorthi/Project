...
class Student:
    def __init__(self,main):
        self.main=main
        self.T_Frame = Frame(self.main, height=50, width=1200, border=2 , background="skyblue", bd=2, relief=GROOVE)
        self.T_Frame.pack()
        self.Title = Label(self.T_Frame, text="Student Management System", font="arial 20 bold", width=1200, bg="skyblue")
        self.Title.pack()

        self.Frame_1=Frame(self.main, height=580, width=400, bd=2, relief="groove", bg="skyblue")
        self.Frame_1.pack(side=LEFT)
        self.Frame_1.pack_propagate(0)

        Label(self.Frame_1, text="Student Details",background="skyblue", font="arial 12 bold").place(x=20, y=20)

        self.Id= Label(self.Frame_1, text="Id",background="skyblue", font="arial 11 bold")
        self.Id.place(x=40, y=60)
        self.Id_Entry = Entry(self.Frame_1, width=40)
        self.Id_Entry.place(x=130, y=60)

        self.Name = Label(self.Frame_1, text="Name", background="skyblue", font="arial 11 bold")
        self.Name.place(x=40, y=100)
        self.Name_Entry = Entry(self.Frame_1, width=40)
        self.Name_Entry.place(x=130, y=100)

        self.Age = Label(self.Frame_1, text="Age", background="skyblue", font="arial 11 bold")
        self.Age.place(x=40, y=140)
        self.Age_Entry = Entry(self.Frame_1, width=40)
        self.Age_Entry.place(x=130, y=140)

        self.DOB = Label(self.Frame_1, text="DOB", background="skyblue", font="arial 11 bold")
        self.DOB.place(x=40, y=180)
        self.DOB_Entry = Entry(self.Frame_1, width=40)
        self.DOB_Entry.place(x=130, y=180)

        self.Gender = Label(self.Frame_1, text="Gender", background="skyblue", font="arial 11 bold")
        self.Gender.place(x=40, y=220)
        self.Gender_Entry = Entry(self.Frame_1, width=40)
        self.Gender_Entry.place(x=130, y=220)

        self.City = Label(self.Frame_1, text="City", background="skyblue", font="arial 11 bold")
        self.City.place(x=40, y=260)
        self.City_Entry = Entry(self.Frame_1, width=40)
        self.City_Entry.place(x=130, y=260)

        # ========================= BUTTONS =========================
        self.Button_Frame=Frame(self.Frame_1, height=250, width=250, relief=GROOVE, bd=2, background="skyblue")
        ...
        self.Frame_2=Frame(self.main, height=580, width=800, bd=2, relief=GROOVE, bg="skyblue")
        self.Frame_2.pack(side=RIGHT)

        self.tree= ttk.Treeview(self.Frame_2, columns=("c1","c2", "c2","c4","c5", "c6"), show="headings", height=30)

        self.tree.column("#1", anchor=CENTER, width=110)
        ...
        self.tree.column("#6", anchor=CENTER, width=160)
    ...
    def Delete(self):
        ...
        selcected_item= self.tree.item(item)['values'][0]
        print(selcected_item)
        c= sqlite3.connect("Student.db")
        cursor= c.cursor()
        cursor.execute("DELETE FROM Student WHERE ID={}".format(selcected_item))
        print("print delection")
        c.commit()
        c.close()
